...
def naturaleza_art(art_id): 
    cur.execute(f"SELECT art_id, title, description, date_published, author, source_name, url, coalesce(nullif(img_url, 'None'), 'https://estaticos.muyinteresante.es/media/cache/760x570_thumb/uploads/images/article/5cff5c385bafe8165cfb8250/plantas-extincion_0.jpg'), content FROM naturaleza where art_id={art_id};")
    art = cur.fetchall()
